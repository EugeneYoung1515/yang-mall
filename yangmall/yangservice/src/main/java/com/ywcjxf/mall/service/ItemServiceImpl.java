...
public class ItemServiceImpl implements ItemService {
    ...
    public Spec getItemFirstSpec(Integer itemId) {
        Spec spec = redisTemplate.opsForValue().get("item:first:spec:"+itemId);
        if(spec!=null){
            return spec;
        }
        spec = specMapper.selectFirstSpecByItemId(itemId);
        redisTemplate.opsForValue().set("item:first:spec:"+itemId,spec,3, TimeUnit.DAYS);
    ...
    public List<Spec> getAllSpecByItemId(Integer itemId) {
        List<Spec> specs = redisTemplate.opsForList().range("item:all:spec:"+itemId,0,-1);
        ...
        if(specs!=null && !specs.isEmpty()){
            redisTemplate.opsForList().leftPushAll("item:all:spec:"+itemId,specs);
            redisTemplate.expire("item:all:spec:"+itemId,3,TimeUnit.DAYS);
        }
        return specs;
    }
    ...
    public Integer getTotalSalesByItemId(Integer itemId) {
        String totalSales = redisTemplateString.opsForValue().get("item:total:sales:"+itemId);
        if(totalSales!=null){
            return Integer.parseInt(totalSales);
        }
        Integer totalSalesInteger = itemMapper.selectTotalSalesById(itemId);
        redisTemplateString.opsForValue().set("item:total:sales:"+itemId,totalSalesInteger+"",3,TimeUnit.DAYS);
